...
> **Project status:** Active development. The application currently includes the complete six-lesson Foundations chapter, five Arrays & Sorting lessons, and seven interactive Linked Structures lessons.
...
- Floyd cycle-detection lab with animated `slow` and `fast` pointers, cyclic tail references, and acyclic null-boundary experiments
- Odd/even middle-finding lab that uses the slow/fast speed ratio instead of a stored list length
...
http://localhost:5173/#linked-cycle-detection
http://localhost:5173/#linked-find-middle
...
Component tests exercise prediction attempts, retry behavior, sorting and search input flows, linked-node identity and pointer movement, catalogue navigation, browser history events, route focus, document titles, and not-found behavior. Registry tests protect route uniqueness, chapter availability, and lesson numbering. The current suite contains 214 passing tests.
