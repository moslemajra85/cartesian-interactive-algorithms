...
describe('catalogue routing', () => {
  ...
  it('maps the chapter catalogue and preserves existing lesson hashes', () => {
    ...
    expect(routeFromHash('#linked-lists')).toEqual({ kind: 'catalogue', chapterId: 'linked-lists' })
    ...
    expect(routeFromHash('#linked-insertion')).toEqual({ kind: 'lesson', slug: 'linked-insertion' })
    expect(routeFromHash('#linked-deletion')).toEqual({ kind: 'lesson', slug: 'linked-deletion' })
    expect(routeFromHash('#linked-traversal')).toEqual({ kind: 'lesson', slug: 'linked-traversal' })
    expect(routeFromHash('#stack-push-pop')).toEqual({ kind: 'lesson', slug: 'stack-push-pop' })
    expect(routeFromHash('#queue-enqueue-dequeue')).toEqual({ kind: 'lesson', slug: 'queue-enqueue-dequeue' })
    expect(routeFromHash('#linked-cycle-detection')).toEqual({ kind: 'lesson', slug: 'linked-cycle-detection' })
    expect(routeFromHash('#linked-find-middle')).toEqual({ kind: 'lesson', slug: 'linked-find-middle' })
  ...
  it('derives document titles from the catalogue', () => {
    ...
    expect(titleForRoute({ kind: 'catalogue', chapterId: 'linked-lists' })).toBe('Linked Structures — Cartesian')
    ...
    expect(titleForRoute({ kind: 'lesson', slug: 'linked-insertion' })).toBe('Linked List Insertion — Cartesian')
    expect(titleForRoute({ kind: 'lesson', slug: 'linked-deletion' })).toBe('Linked List Deletion — Cartesian')
    expect(titleForRoute({ kind: 'lesson', slug: 'linked-traversal' })).toBe('Traversal & Search — Cartesian')
    expect(titleForRoute({ kind: 'lesson', slug: 'stack-push-pop' })).toBe('Stack Push & Pop — Cartesian')
    expect(titleForRoute({ kind: 'lesson', slug: 'queue-enqueue-dequeue' })).toBe('Queue Enqueue & Dequeue — Cartesian')
    expect(titleForRoute({ kind: 'lesson', slug: 'linked-cycle-detection' })).toBe('Two-Pointer Cycle Detection — Cartesian')
    expect(titleForRoute({ kind: 'lesson', slug: 'linked-find-middle' })).toBe('Find the Middle — Cartesian')
